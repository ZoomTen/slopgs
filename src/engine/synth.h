/* synth.h -- channel state, MIDI dispatch, CC/RPN/SysEx. SPEC.md Part 4. */
#ifndef SYNTH_H
#define SYNTH_H
...
typedef struct Channel {
    ...
    /* SPEC.md S4.2.1: the Bank/Program queue's "+0x18 current value" is the
    ...
    uint8_t is_rhythm;    /* SPEC.md S4.8: USE RHYTHM PART gate, simplified per-channel */
    uint8_t mono_mode;
} Channel;

extern Channel g_channels[16];
extern uint8_t g_gs_mode;          /* gates CC0/CC32 storage, SPEC.md S3.1.1/S4.2.2 */
 ...
 * expansion, if any, is the caller's job -- SPEC_GAPS.md). status==0xFF is
 ...
 * SPEC_GAPS.md: msgs_midi's 3-byte signature cannot carry a SysEx message,
