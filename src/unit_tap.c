...
static void emit_line(int pass, const char *fmt, va_list ap, const char *directive) {
    char buf[512]; /* ponytail: fixed cap, longer descriptions truncate; bump if a real test needs more */
...
static void emit_yaml_at(const char *found, const char *wanted, const char *file, int line) {
    printf("  ---\n  found: %s\n  wanted: %s\n  at:\n    file: %s\n    line: %d\n  ...\n",
...
int tap_is_int_at(long long got, long long want, const char *file, int line, const char *fmt, ...) {
    int cond = (got == want);
    va_list ap;
    va_start(ap, fmt);
    emit_line(cond, fmt, ap, NULL);
...
int tap_is_near_at(double got, double want, double tol, const char *file, int line, const char *fmt, ...) {
...
void tap_skip(const char *reason, const char *fmt, ...) {
    char dir[300];
    if (reason && *reason) snprintf(dir, sizeof dir, "SKIP %s", reason);
    else                   snprintf(dir, sizeof dir, "SKIP");
    va_list ap;
    va_start(ap, fmt);
...
void tap_todo_at(int cond, const char *reason, const char *file, int line, const char *fmt, ...) {
    char dir[300];
    if (reason && *reason) snprintf(dir, sizeof dir, "TODO %s", reason);
    else                   snprintf(dir, sizeof dir, "TODO");
    va_list ap;
    va_start(ap, fmt);
    emit_line(cond, fmt, ap, dir);
    va_end(ap);
    (void)file; (void)line; /* TODO test points never carry a YAML block or affect exit status */
}
...
void tap_diag(const char *fmt, ...) {
    ...
    for (const char *p = buf; *p; p++) {
        if (at_line_start) { printf("# "); at_line_start = 0; }
        putchar(*p);
        last_was_nl = (*p == '\n');
        if (last_was_nl) at_line_start = 1;
    }
    if (!last_was_nl) putchar('\n');
}
