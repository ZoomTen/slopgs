...
throughout: **items** (`item1`…`item38`) are discrepancies and gaps between
...
[#item39]
== 2026-07-30 — The `HueArme-Weekend` clap note-on is confirmed present at 77.5482 s; expression is cut to 0 forty-four ms later, which is an untested candidate for the level discrepancy — item39

Two loose ends closed by direct measurement, and one new candidate mechanism
...
=== Candidate mechanism for the ~11 dB level error: `[O]`
...
arrive until 77.6364 s, a further 44 ms after that. The reference's burst
prominence is +3.0 dB against slopgs's +14.2 dB, and a mid-note expression
collapse is the right shape to produce exactly that: the real synth's clap is
cut off part-way through its decay, slopgs's is not.

This is a hypothesis, not a finding. It has not been measured, and it is
independent of the per-voice ceiling that xref:#item35-followup-probe50[probe
50] tests — both could be contributing. What makes it worth chasing before
another gain-scale pass: `CC11` here is toggled on a *sounding* drum voice,
which is the one case probe 28 (expression gate) does not cover, and the
engine's `CC11` handling reaches the voice through the smoothed
`voice_update_gain()` path rather than being latched at note-on. If slopgs
applies that collapse more slowly than the driver, or applies it to the
target rather than the current gain, the tail survives and the burst reads
hotter. **Status: `[O]`, not implemented, no engine change made.**
...
== 2026-07-30 — Probe 50 (`50_gain_ceiling`) finalized to eight sections; attached to xref:#item35[Item 35], reference still not recorded
