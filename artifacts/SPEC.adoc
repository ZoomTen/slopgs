...
`+1200` instruction is `[A]` and not in doubt; what the constant is *for*
is `[O]`, and it interacts with the still-open hundredths-of-a-dB-to-raw-
gain-register mapping (xref:s6-4-5[§6.4.5]).
...
| `0xfbc` | dword×16×12 (16 parts, 12 sub-entries each) | Per-part 12-entry GS Scale Tuning grid, one entry per semitone class; value = `(incoming byte - 0x40)`, i.e. signed offset from a center of `0x40` | GS `DT1` fallthrough case (any address not matched as GS Reset/RCV CHANNEL/USE RHYTHM PART) `[A:0x13797]` | The note-on tuning accumulator `[A:0x12f60]`, indexed `part*12 + note%12` (xref:s3-3-2[§3.3.2]) — the indexing by semitone class is itself what confirms this grid is GS Scale Tuning
...
Twenty-one distinct `[O]` items. Five are **behaviourally significant** —
