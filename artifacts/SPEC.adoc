...
**One discrepancy remains `[O]`.** The law above puts the hard-pan floor at
`PAN[0] − PAN[64]` = −22.03 dB, where the sweep measures **−20.20 dB** and
that figure is a real floor rather than an unmeasured tail. The store is not
in doubt (`movl $0xfffff63c,0x1bfd0` at `0x16a8a`, `0xfffff63c` = −2500), so
this is a confirmed instruction against a confirmed measurement. The
measured table below is what the reference does and is what the engine
implements.
...
* **`[O]`** The exact semantic identity of `region+0x28`, the third
  ...
  is not recovered.
* **`[O]`** Why the hard-pan floor the pan table produces (−22.03 dB,
  `PAN[0] − PAN[64]`) is 1.8 dB deeper than the measured floor of −20.20 dB
  (xref:s3-6[§3.6]). The table's construction and its `PAN[0] = −2500` store
  are both `[A]` and confirmed; the measurement is `[M: probe 25]` and
  explicitly a real floor. The rest of the law reconciles.
...
Twenty distinct `[O]` items. Five are **behaviourally significant** —
...
| xref:o4[4] | Why the pan table's −22.03 dB hard-pan floor is 1.8 dB deeper than the measured −20.20 dB | Behaviourally significant
...
[#o4]
==== Why the pan table's hard-pan floor is 1.8 dB deeper than the measured floor

*Known.* The law is `[A]` and reconciles with the measurement everywhere except the extremes. `0x19c12`–`0x19c2a` reads a single 128-entry table forward and reversed (the bases differ by exactly `127·4`); `0x16a3e` builds it as `PAN[j] = trunc(1000 · log10(j/127))` with `PAN[0] = −2500` stored explicitly at `0x16a8a`. The measured plateau across CC10 48–80 is the `0x18dea` sum clamp pinning both channels at `TABLE[0]`, not a property of the pan law (xref:s3-6[§3.6]).

*Not known.* Why the resulting floor, `PAN[0] − PAN[64]` = −22.03 dB, sits 1.8 dB below the measured −20.20 dB. Both sides are confirmed: the store was re-read byte-for-byte (`movl $0xfffff63c,0x1bfd0`), and `[M: probe 25]` records the floor as real rather than an unmeasured tail.

*How it would be settled.* Re-measure the CC10 extremes against a reference captured at a level that keeps the far channel clear of both the sum clamp and the capture noise floor; failing that, locate a second attenuation term that applies only at the extremes.

*Significance.* Behaviourally significant — 1.8 dB at hard pan is audible, and it is the single term blocking the instruction-level law from replacing the measured table in the engine.
