...
contaminates any 44.1 kHz comparison, is in xref:part1[Part 1] xref:s1-6[§1.6].

[#part1]
== Overview and Implementation Contract

[#s1-1]
=== What this document specifies, and how to read it
...
Appendix T §xref:t1[T.1] and xref:part4[Part 4] xref:s4-0[§4.0] respectively. The specific
...
[#s1-1-2]
==== Table of contents — what each companion section owns
...
* **No send-effect buses of any kind** (the two effects a reader would
  ...
  scanned in full; xref:s2-4-4[§2.4.4] and xref:s6-8[§6.8] both
  ...
  (xref:s4-3[§4.3]) and were separately confirmed acoustically
...
* **No spectral tone-shaping stage.** xref:s6-8[§6.8] read the complete
...
allow (xref:s6-11[§6.11]): its **algorithm class is now known** — a 60-tap-per-phase
...
remain `[O]`**, unrecovered from `kmixer.sys`'s own binary (xref:s6-11-4[§6.11.4]). Two
...
xref:s6-11-2[§6.11.2] independently reaches the same exclusion by reading the code
...
(xref:s6-11-4[§6.11.4]) and the reference capture's own ±1 LSB dither remain uncontrolled
...
from xref:s6-7[§6.7] and Appendix T §xref:t0[T.0], which independently
...
* **Centibel (0.1 dB per unit) units** for the attenuation-style `art1` connections
  ...
  performing the multiply, to avoid signed overflow (xref:s2-4-2[§2.4.2]).
  **Do not confuse this with the separate, finer hundredths-of-a-dB
  (0.01 dB) table used elsewhere** (xref:s3-5[§3.5], the velocity/CC7/master-volume
  ...
  added together. **In the note-on gain sum specifically** (xref:s3-5[§3.5], xref:s3-10[§3.10]),
...
a claim about what the original binary does — §xref:s1-1[§1.1]–1.4 above, and the
...
[#s1-5-3]
==== The public ABI — fixed, and reproduced here verbatim
...
(xref:s4-1[§4.1]) and never itself parses a Standard MIDI
...
* **A single linear arena, with a bump allocator and no `free`.** This
  ...
  xref:s2-1[§2.1] establishes that the original driver reads the entire
...
* The module must not assume any host functions are importable beyond the
  ...
  needs beyond raw memory access (xref:s1-5-1[§1.5.1]) must be linked in from `rt.c`.

'''

[#s1-6]
=== Known divergences and the verification ceiling
...
* **Capture-chain timing offset — negative, file-dependent, not a property
  ...
  (xref:s5-10[§5.10]) `[M]`. The magnitude is stable within a single
...
* **The 22050→44100 upsampling stage is unidentified and is a hard ceiling
  on any 44100 Hz comparison** (xref:s1-3[§1.3], above). Two of the simplest
...
* `field/Strobe-faffaeefafaefae.flac` was also captured over the same loopback
    ...
    treats it as the aliasing-model acid test (xref:part6[Part 6] xref:s6-1[§6.1]).
...
* **This does not reopen or weaken the findings already recorded about
    ...
    xref:SPEC_GAPS.adoc#item15[item 15]), together with the onset-timing accuracy check performed
...
[#s1-7]
=== Two points settled by direct instruction reading
...
. **The note-setup phase-increment store (`0x18ef4`) uses `voice+0x28` as a
...
----
   ...
   reads at entry and rewrites at exit as its persistent phase step (xref:s6-4[§6.4] of
   the render section). `voice+0x44` (the phase accumulator) is zeroed at
   note start at `0x18f7d`, in the middle of this same
   instruction run. xref:s1-3[§1.3] (data-flow) and the render/voice sections below are
   written consistently with this account.
. **The pitch-ratio table at `0x1ad00` has domain n = −100..100 (201
...
itself because (verified in xref:s2-7[§2.7]'s cross-check script) every chunk in the real
...
. **Real file data** (xref:s2-11[§2.11] cross-check script, run against `gm.dls`): dest
   ...
   `[D:gm.dls]` (exact script output in xref:s2-11[§2.11]). These are not "always zero" —
...
[#s2-4-3-1]
==== Runtime LFO→PITCH (vibrato) reimplementation — `[M: probe 06]`
...
time-domain helpers, xref:s2-4-2[§2.4.2]) was never located in this project's
...
* **Depth**: only the CC1-gated connection (`usControl==0x0081`) is applied,
  ...
  256-entry sine table (xref:t5[T.5]) sampled at the per-voice LFO phase. The sibling
...
[#s2-4-3-2]
==== Runtime decay key-follow reimplementation — `[M: field/town.mid]`, normalization `[I]`
...
* **Cross-check on a second, independently measured instrument:** Piano 1
  note 60 (`SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item15[§15]'s own measured reference, −7.14 dB/s). Base
  6386.5 tc = 40.0 s gives 2.50 dB/s with no key-follow; with it, 13.63 s =
  **7.34 dB/s**. That resolves xref:SPEC_GAPS.adoc#item15[§15]'s open "still ~2.9× too slow" residual and
  explains why `FITTED.md` xref:FITTED.adoc#entry1[Entry 1]'s global 2.85× decay multiplier fixed
...
* **Normalization — measured `[M: probe 35]`, partially settled.** The driver
  ...
  (xref:part5[Part 5] `+0x13c`, `[O]`), so the divisor was `[I]` from DLS-1's own
...
* **Decay *shape* — `[M: probe 35]`.** With the per-key duration correct, the
  ...
  the 100 dB xref:s3-4-2[S3.4.2]'s shared `exp_coef` assumes; shipped as
  `DECAY_RATE_MULT = 0.965` (`voice.c`). Release is untouched — xref:part5[Part 5] xref:s5-6[S5.6]'s
...
* **Corpus effect:** mean spectral residual −24.55 → −28.18 dB on key-follow
  ...
  0.903 → 0.909. Regressions named in `FITTED.md` xref:FITTED.adoc#entry15[Entry 15].
...
[#s2-4-4]
==== Destinations/sources explicitly, confirmedly ignored
...
[#s2-6]
=== `wsmp` handling — summary (full byte table in §2.3.4)
...
[#s2-7-3]
==== `gm.dls`'s actual wave inventory — measured, not assumed

Cross-check script output (xref:s2-11[§2.11], full script and stdout reproduced there):
...
stated in the assignment briefing; I ran my own independent script (xref:s2-11[§2.11]) and
...
[#s2-8]
=== `ptbl` / wave-pool mechanics — fully resolved
...
[#s2-8-2]
==== List-append order — settled definitively
...
[#s2-8-3]
==== The critical, previously-undocumented fixup step — inside the
...
[#s2-9-1]
==== Region fields (all individually re-verified this pass unless marked)
...
[#s2-9-3]
==== Instrument and collection fields (established this pass)
...
[#s2-10]
=== `insh` locale packing — verified against real `gm.dls` bytes

Formula (xref:s2-3-1[§2.3.1]): `locale = ulInstrument | ((ulBank&0x7f)<<7) |
((ulBank&0x7f00)<<6) | (drum?0x80000000:0)`.

`[D:gm.dls]` (verified by the xref:s2-11[§2.11] script, which independently computes
...
`insh` per-instrument dump in xref:s2-11[§2.11]'s script output): every one of the 235
...
[#s2-11]
=== `gm.dls` inventory cross-check — script and real output
...
* `ptbl.cCues == 495 == wave count`, and **every one of the 495 declared
  ...
  precondition for xref:s2-8[§2.8]'s resolution mechanism to work at all on this file.
* art1 destinations 0x020a/0x030e genuinely occur with non-zero `lScale` in
  real data (104/203 and 4/223 respectively) — see xref:s2-4-3[§2.4.3]'s correction.

'''

[#s2-12]
=== Summary of key findings, this section

. `usUnityNote` is byte-truncated at **both** wave and region scope.
. `art1`'s connection array starts at `chunk_data+8` (standard placement);
   `chunk_data+0x10` is an internal cursor's offset into the first block, not
   the array's start.
. The `art1` destination→field-offset mappings for source=0 are given in
   full in xref:s2-4-3[§2.4.3], with fresh VMA citations (e.g. `0x0104`→wave+0x40,
   `0x0105`→+0x48, `0x0207`→+0x28, `0x030a`→+0x00, `0x030c`→low word).
. `usSource==1`(LFO)→`usDestination==0x0001`(ATTENUATION) rows exist for
   both `usControl==0` and `==0x81` — xref:s2-4-3[§2.4.3].
. `0x020a`/`0x030e` are **present** in the dispatch chain and are not
   always `lScale==0` — xref:s2-4-3[§2.4.3], with both a disassembly citation and real-file
   counts showing non-zero `lScale` in over half the occurrences.
. `edit` has **five** code sites, including the shared `lart`-scope site at
   `0x157c3`, which writes to the articulation-block object, not
   region/instrument directly — xref:s2-3-10[§2.3.10].
. `fmt ` stores `nSamplesPerSec` (not `nBlockAlign`) at wave+8.
. `fmt ` enforces `nChannels==1`.
. Wave-level `fulOptions` is not dead — it gates the 16→8-bit
   sample-storage-reduction path (xref:s2-7-2[§2.7.2]/xref:s2-8[§2.8]); this is true only at wave scope,
   not at region scope.
. The table at file offset `0x1c1d0` is not a WAV-format A-law/µ-law
    ...
    instruction trace, xref:s2-7-2[§2.7.2].
. The `ptbl`/`wvpl` end-to-end resolution mechanism is fully traced: the
    ...
    in-memory pointer at the real wave chunk's bytes — xref:s2-8[§2.8]. The one
    remaining open link (who actually decodes `fmt`/`data`/`wsmp` from that
    pointer) is precisely scoped in xref:s2-8-6[§2.8.6].
. Wave-level `wsmp` enforces `ulLoopType==0` as a hard requirement (a parse
    error otherwise); region-level `wsmp` never reads `ulLoopType` at all —
    a wave/region asymmetry.
. `data` before `fmt ` is a hard parse error (`0x80041389`), not merely an
    unchecked ordering quirk.

'''

[#s2-13]
=== Open items — precisely stated, not guessed
...
* **[O]** The ultimate origin/value of the `flagByte` argument threaded
  ...
  policy, xref:s2-7-2[§2.7.2]) for a stock `gm.dls` load — not traced back to its source
...
[#s2-14]
=== Contamination / scope-discipline check (self-check, run on this file)
...
`\Registry\Machine\Software\Microsoft\DirectMusic` quoted in xref:s2-1[§2.1] — this is the
...
tone-shaping destination IDs are discussed in xref:s2-4-4[§2.4.4] without using the noun
...
into each instrument's own `+0x10` field from `insh` (companion section, xref:s2-3-1[§2.3.1]).
...
region-selection time (companion section, xref:s2-3-3[§2.3.3]), confirmed again here by
...
FIFO tail-append, xref:s3-8-2[§3.8.2] of that section, reused here for regions via the same
...
an instrument-level `lart` to share `[D:gm.dls]` (both counts, script in xref:s3-11[§3.11]).
...
[#s3-3-2]
==== Unity note, fine tune, key, and pitch-bend combination
...
xref:s3-3-4[§3.3.4]) — **read here unconditionally, regardless of the region's own
...
fields, companion section xref:s3-3-4[§3.3.4]/xref:s3-9[§3.9]) into Q12 fixed point for the phase
accumulator's own bounds checks. **`region[N]` denotes the absolute region
offset `region+N`** throughout this section (matching xref:s2-9-1[§2.9.1]'s field table
...
— script and full output in xref:s3-7[§3.7]; this reproduces the assignment's stated
...
formula, xref:s3-4-1[§3.4.1], already establishes an exponential *time* law; combined with
...
volume/velocity tables (xref:s3-5[§3.5]) two different ways:
...
directly from `0x19bfe`–`0x19c2a`, address arithmetic confirmed in xref:s3-7[§3.7]'s
...
choice — see `FITTED.md` xref:FITTED.adoc#entry5[Entry 5]. The physical L/R channel assignment
...
[#s3-7]
=== Two decoder quirks (restated briefly; owned in full by the DLS section)
...
`region+0x31` (`usKeyGroup`, stored from `rgnh+0xa`, companion section xref:s2-3-3[§2.3.3])
...
[#s3-9]
=== Verification scripts and output
...
[#s3-9-1]
==== `verify_art_pitch_tables.py` — the three pitch tables (T1/T2/T3)
...
[#s3-9-2]
==== `verify_art_velocity.py` — the velocity/attenuation table over all 128 entries
...
[#s3-9-3]
==== `verify_art_gmdls_articulation.py` — 552 distinct timecents, ULP margin, 7 key groups
...
[#s3-9-4]
==== `verify_art_pan_law.py` — the reversed-Table-B / direct-Table-A pan mechanism
...
* **`[O]`** The exact scaling arithmetic that converts a channel's raw
  ...
  cents) into the cents figure consumed at xref:s3-3-2[§3.3.2] — the default values
...
* **`[O]`** Which physical output channel (left/right) receives the pan
  law's `gainA` versus `gainB` (xref:s3-6[§3.6]) — the two curve identities and the
...
* **`[O]`** Table B's own selection-vs-Table-A-for-velocity code remains
  ...
  the pan law, xref:s3-6[§3.6]) without closing the original velocity-selection question.
...
| `0xf78` | dword (device-global, not per-channel) | Master Volume attenuation, in hundredths of a dB (dB = value/100) (`0`=unity, from the same curve as note-on velocity, xref:s4-4[§4.4]/xref:s4-9[§4.9]) | SysEx Master Volume `[A:0x12d2d]` | consumed by DSP/gain code outside this section `[O]`
...
| `0x12fc` | byte×16 | RCV CHANNEL: which physical MIDI channel index (0–15) feeds internal Part *i* | `ResetDevice` (from the static table, xref:s4-6[§4.6]) `[A:0x123aa]`; GS SysEx RCV CHANNEL `[A:0x137ae]` | `ShortMsg`'s per-Part CC/note dispatch loop (matches incoming channel against this array) `[A:0x13267]`
...
| `0xf54` | dword (device-global) | GS-mode flag: gates CC0/CC32 storage (xref:s4-3[§4.3], xref:s4-4[§4.4]) | GS Reset sets `1` `[A:0x137f4]`; GM System On clears `0` `[A:0x136f5]`; `ResetDevice` clears `0` `[A:0x123c5]` | CC0 `[A:0x1341e]`, CC32 `[A:0x13492]`; RCV CHANNEL/USE RHYTHM PART SysEx gate `[A:0x137a2]`,`[A:0x1376e]`
|===
...
| 121 | Reset All Controllers | Implemented — re-schedules Volume=100, Pan=64, Expression=127, Pitch Bend=8192, Modulation=0 (same values as xref:s4-6[§4.6]'s `ResetAllChannelControllers`) plus a per-channel sustain-release event; does **not** touch Bank/Program or the RPN/NRPN-select register | `0x1351f` `[A]` | n/a
...
| `F0 41 <any> 42 12 40 1x yy ... F7` (any other address in the `40 1x` family), gated on GS-mode flag | Per-Part 12-entry tuning-grid write (xref:s4-2-2[§4.2.2], `device+0xfbc`) | 12 iterations, one per semitone class, each storing `byte - 0x40` | `[A:0x1376c]` (gate test), `[A:0x13797]` (store loop)
...
`device+0x20`, the same object also passed to `FindInstrument`, xref:s4-7[§4.7]/xref:s4-8[§4.8]) — `[O]`.
...
. `device+0xf78` (Master Volume attenuation) and `device+0xf7c`/`0x12bc`
   ...
   scope.
. The 16×12-entry tuning grid at `device+0xfbc` (written by the GS `DT1`
...
. `0x151b0`'s exact internal effect (called from both `ShortMsg`'s
...
`verify_voice_*.py` (RE workspace not retained, xref:s5-10[§5.10]), which parsed
...
| `+0x68` | 0x28 bytes | Release-envelope-segment sub-object (own `+0x10`/`+0x14` = current ramp rate 64-bit; own `+0x28`/`+0x2c` = target) — configured by `0x197dc` (note-off, normal rate) or `0x19834` (fast/choke, rate-clamped) | see xref:s5-6[§5.6] | —
...
[#s5-1-1]
==== Voice `+0x8`: the pitch-LFO run-state object, not an EG1 segment — CORRECTS a prior pass of this section

An earlier pass of this section, chasing the same decay-model discrepancy
`SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item23[item 23] describes, misidentified `voice+0x8`'s setup call
...
storage location** (xref:s2-4-3[§2.4.3]'s table: DLS dest `0x0104` → `wave+0x40`,
...
[#s5-1-2]
==== Voice `+0x68`: the actual EG1 (amplitude) state machine — LOCATED this pass, resolves most of `SPEC_GAPS.md` item 23

`voice+0x68` (this section already names it "release-envelope-segment,"
from observing it gets *reconfigured* at note-off, xref:s5-6[§5.6]) is in fact the
...
`articBlock+0x20` is confirmed EG1 attack (xref:s2-4-3[§2.4.3]'s table); the callee,
...
service routine `0x13054`, xref:s3-4-2[§3.4.2]/xref:s6-6[§6.6]'s confirmed block cadence) does:
...
**This is the mechanism `SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item23[item 23] was missing.** It is not
...
xref:part2[Part 2] correctly confirmed as the *storage* format — raw, unscaled, `[A:
...
against `SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item23[item 23]'s own measured data (`probes/39`/`41`): the
...
`SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item23[item 23] for the full comparison table — that document is
the one to update with this mechanism's fit quality, not this section).

[#s5-1-2-1]
==== The decay segment's SHAPE, not just its target and duration — CORRECTS this section's own first pass, same day
...
translation of the pseudocode above — re-reading xref:s5-1-2[§5.1.2]'s own derivation
...
established for RELEASE (xref:s3-4-2[§3.4.2], the probe-04/xref:s5-6[S5.6] finding that motivated
`exp_coef_scaled`'s "100dB-over-`seconds`" calibration in the first place —
see `SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item15[item 15]). **Decay should use release's own existing
...
xref:s3-8-2[§3.8.2]'s pre-existing pseudocode (`[A:0x19a2c]`, `[A:0x19aa4]`) labels
...
[#s5-5]
=== Resolution of the pool-size question
...
`18`, xref:s5-10[§5.10] script), but the **exact magnitude differs between probe files**
...
burst self-terminates because recycling catches up," xref:s5-5[§5.5]) are marked `[I]`
...
* The object pointed to by voice `+0x104` — only 4 of its own offsets
  ...
  xref:s5-1-1[§5.1.1]/xref:s5-1-2[§5.1.2]**: `+0x40` is LFO_FREQUENCY (this is the confirmed
  articulation-block object, not a separate one — see xref:s2-4-3[§2.4.3]'s own table),
  and `+0x20` onward is the confirmed EG1 attack/decay/release/sustain
  block. `+0x0` (what feeds `voice+0x38`, confirmed EG2 by xref:s3-8-2[§3.8.2]'s
...
* The exact scheduling semantics of the `+0x108`/`+0x10c` and `+0x118`/
  ...
  here. **Resolved, xref:s5-1-2[§5.1.2]**: `voice+0x68`'s own decay-region progress
...
* *Why* the driver splits the pool into 48+6 rather than one flat 54-deep
  ...
  xref:s5-2[§5.2]–xref:s5-5[§5.5]; the *design intent* is not).
* The precise numeric time unit of `0x19834`'s divisor constant `70` (xref:s5-6[§5.6])
...
* The exact byte-for-byte confirmation of `rgnh`+`0xa` against the DLS
  RIFF/RGNHEADER specification (xref:s5-8[§5.8]) was not independently re-derived here
...
[#s6-0]
=== Scope and how to read the provenance tags
...
[#s6-4-1]
==== Call arguments (confirmed by direct trace of both shapes)
...
real driver's output instead of flattening). Two confirmed facts in xref:s6-4-5[§6.4.5]
...
item above). Below that ceiling the squared volume law (xref:s3-5[§3.5]/xref:s6-5[§6.5]) applies
...
xref:s6-4-5[§6.4.5]'s confirmed per-channel L/R gain state) to `GAIN_CEILING =
...
rest of xref:part6[Part 6] — refers to `swmidi.sys`. The two address spaces are
...
`swmidi.sys` itself renders at a single fixed rate, 22050 Hz (xref:s6-1[§6.1]); it has
...
not directly observed in this pass (xref:s6-11-4[§6.11.4]).

[#s6-11-2]
==== Zero-order hold and linear interpolation are excluded, not merely unconfirmed
...
[#part7]
== Register of the Unreached and the Unknown
...
[#s7-a]
=== Part A — The unreached-code register

[#s7-a-1]
==== Method: independent re-derivation, not a restatement
...
. Parse the `.text`-section disassembly (VMA `0x10380`–`0x117b4`) and
   the PAGE-section/`.init`-section disassembly into address→instruction maps.
. **Entry points** = every `.text` address that is the target of a direct
   ...
   one).
. Build the intra-`.text` graph: every `call`/`jmp`/`jcc` edge plus a
   fallthrough edge from each non-terminal instruction to the next (skipping
   `int3` alignment padding, which never executes).
. BFS/DFS closure from the entry-point set.
. Candidate "function starts" = the union of (a) every address that is a
...
. Report which function starts are outside the closure, and — critically —
...
re-verification and are corrected below (xref:s7-a-3[§7.A.3]); they do not change the
headline result.

[#s7-a-3]
==== Two corrections to the brief's own supporting detail, found in the course of verification
...
| `0x104c0`–`0x104c1` (2 bytes) | `__cdecl` forwarder stub for `pow`: `jmp 0x104df` | Zero incoming references anywhere in the binary — every real caller (all inside `pow`'s own table-building call sites in PAGE, and the four internal call sites inside `pow`'s own body) targets `0x104c4` directly, two bytes past this stub, skipping over it entirely | `[A:0x104c0]` (verified: no `call`/`jmp` operand anywhere resolves to `0x104c0`); confirmed by the reachability script, xref:s7-a-2[§7.A.2] | Would become reachable only if some caller were changed to target `0x104c0` instead of `0x104c4` — i.e. never, for this exact binary; there is no configuration, registry key, or runtime condition that selects it
| `0x104c2`–`0x104c3` (2 bytes) | a two-byte no-op (self-move, the standard hot-patchable-prologue idiom), the true start of the function whose *entered* address is `0x104c4` | Not a call target either (every caller targets `0x104c4`, past the pad); this is normal, expected shape for a hot-patchable prologue, not a fourth dead function | `[A:0x104c2]`; reachability script confirms `0x104c4 ∈ reachable`, `0x104c2 ∉ reachable`, and explains why (xref:s7-a-2[§7.A.2]) | N/A — this is inert padding by design, not code an implementer would ever port either way
| `0x10710`–`0x10ad9` | The CRT `exp()` implementation, plus five private helper subroutines it alone calls (`0x1085e` — an `fabs`/`fcompp` overflow/underflow classifier; `0x108a1` — an `frndint`-based integer/fraction classifier; `0x108d5`–`0x10906` — an `fsave`/`frstor` wrapper around a second `0x10d46` call; `0x10ac9` — a small tail helper; plus roughly 40 internal labels reached only by conditional/unconditional jumps within `exp()`'s own body, including one indirect jump-table (switch) dispatch at `0x10a01` whose jump-table entries were not resolved — they lie inside this same dead island regardless of their exact targets) | Zero incoming references to `0x10710` itself from anywhere; this driver never calls `exp()` — every `lScale`→duration conversion in the timing-conversion code (xref:s3-4-1[§3.4.1] of the companion articulation section) goes through `pow(2.0, x)` at `0x104c4`, never through `exp()` | `[A:0x10710]` (verified: no reference anywhere); full private-helper enumeration in xref:s7-a-2[§7.A.2]'s script output; cross-checked against the companion articulation section's own citation of `0x104c4`/`fyl2x` as the only float path exercised for timecent conversion (xref:part3[Part 3] xref:s3-4-1[§3.4.1]) | Would become reachable only if some future code change called `exp()` directly instead of `pow(2.0, x)` — not applicable to this exact binary
| `0x10910`–`0x10a00` (at least; boundary with the `exp()` island not fully separated because both regions are dead and low-priority to disentangle further) | A two-operand IEEE-754 special-case classifier (per-operand `fxam`+`xlat`-nibble classification feeding an indirect jump-table dispatch at `0x10975`) | Zero incoming references from anywhere in the binary, including from `0x10710` — an independently orphaned island, not `exp()`'s own callee (xref:s7-a-3[§7.A.3]) | `[A:0x10910]` | Would become reachable only if some caller referenced it directly (e.g. a two-argument CRT function such as `atan2`/`fmod` that this driver never actually calls) — not applicable here
| `0x104e8`–`0x104c4`+body, `0x10ae0`, `0x10b40`, `0x10d46`, `0x10cd1`, `0x10eaf`, `0x10fef`, `0x11008`, `0x110ab`, `0x110dc`, `0x1113e`, `0x11201`, `0x1125d`, `0x1149e`, `0x116c9`, `0x116f9`, `0x11701`, `0x11716`, `0x1172c`, `0x11757`, `0x117b4` | `pow`'s own exceptional-input paths: NaN/Infinity/denormal classification, `_fpieee_flt`-style error reporting, `errno` setting, and `_controlfp`-adjacent FPU-state helpers | **These ARE reached** by the closure (confirmed xref:s7-a-2[§7.A.2]) — they are not dead code. They are included here because, for the actual inputs this synthesizer ever evaluates `pow` on (finite, normal-range `double`s drawn from `gm.dls`'s timecent domain, already shown by the companion articulation section's own 552-distinct-value/1,367,824-ULP-margin check to never land near a boundary condition, and from the velocity-table exponent-4 domain, `v/127 ∈ [0,1]`), **none of these branches is ever entered at runtime**. They are live code, unreached for this specific input domain — a materially different claim from dead code | `[A]` reachability (xref:s7-a-2[§7.A.2]); domain argument `[I]`, based on the articulation section's own exhaustive enumeration of every timecent value `gm.dls` actually supplies (xref:part3[Part 3] xref:s3-4-1[§3.4.1]) and the velocity table's fixed `v=0..127` domain (Appendix T §xref:t2[T.2]) | **An implementation may omit this entire family**, provided it documents that its own `pow` need only handle finite normal inputs — the condition under which it would need to be ported is a future DLS collection (not `gm.dls`) supplying a timecent value producing a non-finite intermediate, which none of `gm.dls`'s 552 distinct values do
| `0x16eb0`, `0x17018`, `0x1748c`, `0x17790`, `0x17a24` (scalar/non-MMX mixer, plus siblings `0x17bde`, `0x17e32`, `0x175b6`, `0x176d0`, `0x17132`, `0x17268`, `0x17326`) | The scalar (non-MMX) render/mixer family, structurally parallel to the four MMX functions (two-tap linear interpolation, same manual saturating-add idiom) | Path selection is by CPUID only: EFLAGS.ID toggle `[A:0x1a54e]`, `cpuid` `[A:0x1a565]`, a test of the CPUID.1 feature-flags result against mask `0x800000` (bit 23, MMX) `[A:0x1a568]`. **There is no registry override** — the string `MMXDisabled` occurs nowhere in `swmidi.sys`, ASCII or UTF-16LE (exhaustive byte-membership search, `verify_render_dispatch.py`, both checks PASS). On any CPU manufactured after 1997 the MMX bit is unconditionally set, so the scalar family is never selected | `[A:0x19316]`–`[A:0x19379]` (dispatch chain resolves to the MMX callee for every reachable format code); xref:part6[Part 6] xref:s6-2[§6.2] and its `verify_render_dispatch.py` (18/18 checks pass) | Would become reachable only on a CPU lacking the MMX CPUID bit — no real x86 CPU manufactured after 1997 lacks it, and there is no software switch. Whether the scalar path's output is bit-identical to the MMX path for the same input is `[O]` (xref:s7-b[§7.B]) — not required to answer to justify omitting it, since it is never selected on any real deployment target
| `0x19e1e`–`0x19fcc` (function A, MMX, 8-bit source, mono), `0x19fd2`–`0x1a1b1` (function B, MMX, 8-bit source, stereo) | The two MMX mixer variants that read an 8-bit-per-sample PCM source | `gm.dls` is 495/495 sixteen-bit mono PCM waves (`[D:gm.dls]`, re-verified independently by this section, xref:s7-a-5[§7.A.5], and by xref:part2[Part 2] xref:s2-11[§2.11]/xref:part6[Part 6] xref:s6-9[§6.9] `verify_render_format.py`, 21/21 checks) — the format-code dispatcher only ever emits codes `0x71`(B)/`0x72`(D) for the two output-arity choices, and every `gm.dls` wave's own `wBitsPerSample=16` forces the 16-bit branch of that dispatch, so A/B are real, dispatchable code that is simply never selected for any voice sourced from this collection | `[A:0x19316]`-`[0x1935c]` (dispatch chain); `[D:gm.dls]` wave-format inventory | Becomes reachable the moment a DLS collection supplies an 8-bit PCM wave (`wBitsPerSample=8` in `fmt `, a format this same parser fully supports — xref:part2[Part 2] xref:s2-7-1[§2.7.1]/xref:s2-7-2[§2.7.2] documents the 8-bit load path in full) — this is a property of the *loaded collection*, not of the driver, and would flip immediately for a non-`gm.dls` sound bank
| Any DLS articulation `(usSource, usControl, usDestination)` combination the parser's dispatch chain recognizes but `gm.dls` never supplies a connection for | Two specific combinations were checked directly against every `art1` connection block in the file (both instrument-level and region-level `lart`, 7,451 blocks total — matching xref:part6[Part 6]'s own independently-obtained count exactly, confirming this section's re-scan is consistent): **`(usSource=1 LFO, usControl=0x0081 CC1, usDestination=0x0001 ATTENUATION)`** — tremolo depth driven by the mod wheel — **0 of 7,451**; **`(usSource=2 KEYONVELOCITY, usDestination=0x0001 ATTENUATION)`** — a DLS-native velocity→attenuation connection (distinct from this driver's own hardcoded note-on velocity term, which is a separate mechanism entirely, xref:s6-5[§6.5] of xref:part3[Part 3]) — **0 of 7,451** | Both combinations are explicitly recognized and correctly dispatched by the `art1` decoder (xref:part2[Part 2] xref:s2-4-3[§2.4.3], Source=1 and Source=2 tables) — this is real, exercised-by-other-inputs code (the *decoder function itself* is exercised thousands of times per load by the other combinations in the same table), it is simply never fed these two specific `(source,control,dest)` triples by this collection's authored content | `[D:gm.dls]`, independent re-scan script (this section, full-file both-scope scan; 7,451 total blocks cross-checked against xref:part6[Part 6]'s `verify_render_format.py` count of 7,451) | Becomes reachable the moment any instrument or region in a loaded collection authors a connection with exactly one of these two `(source,control,destination)` triples — a content fact, not a driver limitation; the decoder already handles it correctly, an implementer following the full `art1` table (xref:part2[Part 2] xref:s2-4-3[§2.4.3]) already covers this case without any special handling
|===

[#s7-a-5]
==== Independent verification of the `art1`-content claim (this section's own script)
...
[#s7-b]
=== Part B — The open-questions register
...
as open — see the note under xref:s7-b-1[§7.B.1] and xref:s7-b-2[§7.B.2].

[#s7-b-1]
==== Two items closed by cross-reference, not listed as open below

* **The exact formula implemented by the timecent-conversion helpers
  `0x15364`/`0x153aa`** — xref:part2[Part 2]'s own DLS-parsing section flagged this
  ...
  helper (xref:part3[Part 3] xref:s3-4-1[§3.4.1], `[A:0x15364]` `[A:0x153aa]`). **This is
  settled, not open** — the xref:part2[Part 2] flag simply predates the articulation
  section's own pass over the same two VMAs.
* **The instrument `(bank,program,drum)`→instrument-object lookup function**
  — xref:part2[Part 2] flagged this `[O]` as "outside PAGE `0x142f8`–`0x1667f`". The
  ...
  (xref:part3[Part 3] xref:s3-1-2[§3.1.2]–xref:s3-1-4[§3.1.4]). **This is settled, not open.**
...
* **Reachability script**: `verify_text_reachability.py` (RE workspace not retained), ran clean, asserted the three-unreachable result (`0x104c0`, `0x10710`, `0x10910`), output pasted in full above (xref:s7-a-2[§7.A.2]).
...
* **"Settled" items marked open instead**: none of the briefing's listed settled items failed verification against the companion sections — all thirteen checked out as stated or stronger. Two *supporting details* inside the reachability claim itself did not survive re-verification and are corrected in xref:s7-a-3[§7.A.3] (0x10910's alleged caller; one of the four call sites attributed to `pow`'s own body). Two items framed as open by the briefing (velocity-table selection, xref:s7-b[§7.B] item 3; two DLS-parser open items, xref:s7-b-1[§7.B.1]) were found to be substantially more settled than framed, and are presented with that narrowing made explicit rather than left as flatly open.
...
functions for tables C/D/E (§xref:t4[T.4]–§xref:t6[T.6], sine and two of the `log10` curves) call
...
one table (§xref:t2[T.2], velocity-attenuation) that calls `pow()`; for the others, the
...
[#t1]
=== PE section map and VMA→file-offset conversion **(a)**

Parsed from scratch with `struct.unpack` against the DOS/PE/section
headers (script §xref:t1[T.1], pasted below). Image base `0x10000`; `SectionAlignment`
...
[#t2]
=== Velocity → attenuation table (`0x1c9d0`, 128 × int32; dB = table[v] / 100.0) **(a)**

**This table is built at runtime**, not stored on disk — every one of its 512
bytes is `0x00` in the file image (proven below, script §xref:t4[T.4]). It is built once
...
task** — this appendix (§xref:t0[T.0], opening note) explicitly has no audio reference
...
`trunc` = truncation toward zero (§xref:t0[T.0]). Recomputed independently in Python
double precision, using the **exact stored float32/float64 constants** read
from `.rdata` (not idealized values — see §xref:t4[T.4] for why this matters):
...
[#t3]
=== Linear velocity table (`0x1bfd4`, 127 × int32, plus scalar `0x1bfd0`) **(a)**

Also built at runtime (508 on-disk bytes, all zero — script §xref:t4[T.4]), by PAGE
...
recomputation (script §xref:t6[T.6], below).
...
[#t4]
=== Table C — `0x1a9d8`, 201 × int16, indices 0..200 **(a)**
...
[#t5]
=== Table D — `0x1a7d8`, 256 × int16, sine LFO **(a)**
...
[#t6]
=== Table E — `0x1c1d0`, 2048 × uint8, log-companding curve **(a)**
...
[#t6-1]
==== Layout cross-check (bonus corroboration, (a))
...
[#t7]
=== `.rdata` DSP scalar constant pool `0x11c98`–`0x11d1c` **(a)**
...
[#t8]
=== Table 0 — channel processing/init order **(a)**
