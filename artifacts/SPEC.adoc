...
[#s2-4-3-2]
==== Runtime decay key-follow — `[M: field/town.mid]`, normalization `[I]`

The parsing above is `[A]`. The consumption is `[O]` — the driver stores the
full-scale high word and the code that reads it was not located. The
behavioural model that reproduces the reference renders is:

----
decay_tc = authored_decay_tc + kf_tc * key / 128
----

applied at note-on to both EG1 and EG2. Higher notes decay faster.
...
|===
  ...
  form to 3.5% `[M: probe 35]`. `/127` vs. `/128` is **not** separated by
  this measurement — the measured divisor is 126.5 ± 0.8, and the two differ
  by 0.8% in slope and 1.4% in offset at key 96. `/128` is the natural `>>7`
  for integer code and is what this specification prescribes `[I]`.
* **Decay rate scale — `[M: probe 35]`.** With the per-key duration correct,
  a single uniform scale factor remains across all 17 measured notes: the
  decay segment traverses **96.5 dB over `seconds`**, not the 100 dB that
  xref:s3-4-2[§3.4.2]'s release calibration uses (sd 0.005, three
  instruments, rates spanning 3.8–27 dB/s, no trend against key or
  instrument). The fast release (xref:part5[Part 5] xref:s5-6[§5.6]) still
  matches 100 dB. That 96.5 ≈ 96.33 dB, the 16-bit floor, is **`[O]`** — it
  is 1.4 sd from the measurement and not confirmed.
...
(200 cents = 2 semitones) `[A:0x16e3d]` `[A:0x16e44]`.
The exact instruction that scales the raw 14-bit bend value by the range
before it reaches this cents sum is **`[O]`**; the
...
Amplitude (EG1) release is exponential in shape: the driver's own timing
formula (xref:s3-4-1[§3.4.1]) establishes an exponential *time* law `[A]`,
and an exponential fits rendered release tails to `R² = 0.999` `[M]`.
...
atten_sum += (int16)region[0x24]; // 0x19bc9: region's own wsmp attenuation (§2.3.4)
----

`[A:0x19b94]`–`[A:0x19bc9]`, full trace. `velocity` here is
...
**The gain law is `[M: probe 25]`, not `[A]`.** The instructions that follow
the clamp (`0x19c12`–`0x19c2a`) read as a two-table asymmetric law —
`gainA = *(int32*)(0x1c1cc - pan*4)` (the linear/√-law table indexed in
reverse) and `gainB = table_1c9d0[pan]` (the squared-law table) — but that
formula predicts real attenuation on *both* channels at center pan
(≈−3.0 dB and ≈−11.9 dB, a mid-pan power dip) and full silence at the
extremes, and the reference audio shows neither. The law below is what the
reference actually does; reconciling it with `0x19c12`–`0x19c2a` is **`[O]`**.
...
each of the nine notes, all agreeing to within 0.03 dB:
...
* **`[O]`** The exact scaling arithmetic that converts a channel's raw
  ...
  themselves are `[A]`; the scaling instruction is not located.
...
* **`[O]`** The exact real-time magnitude of the choke/steal release-time
  ...
  resolved.
* **`[O]`** How `0x19c12`–``0x19c2a``'s two-table read reconciles with the
  measured pan curve (xref:s3-6[§3.6]). The instructions are `[A]`; the
  behaviour they produce is not what the reference renders show.
...
| `0xf7c` | dword×16 | RPN1 (Channel Fine Tuning) result, in cents | RPN1 calc, shared tail for CC6/CC38 `[A:0x1348a]` | `[O]` — consumer not located
...
| `0x12bc` | dword×16 | RPN2 (Channel Coarse Tuning) result, in cents | RPN2 calc (CC6 only, param==2) `[A:0x133f2]` | `[O]` — consumer not located
...
| `F0 41 <any> 42 12 40 1x 15 <data> <chk> F7` | USE RHYTHM PART (per-Part rhythm-map assignment), gated on GS-mode flag | Writes `data` into `device+0x130c+part*1`, then enters the shared reset tail and calls `ResetAllChannelControllers` — releasing every sounding voice and resetting every controller on all 16 channels. It does **not** call `ResetAllProgramsAndRhythmGroups`, so Bank/Program survive | `[A:0x1375e]` (address match), `[A:0x137a2]` (gate test), `[A:0x137ae]` (store), `[A:0x137b5]` (`jmp` into the tail), `[A:0x13807]` (tail gate, false on this path)
...
On or a System Reset (`0xFF`) re-acquires — `[I]`, inferred from the
teardown; the refcount at `[A:0x16066]` releases the collection only if no
other reference is outstanding, and whether a sounding voice holds one is
`[O]`.
...
| `+0x18` (WORD pair) | `+0x38` | velocity→attack/decay duration scales — see below
...
// note-on arguments; both trace toward the note's velocity, [I]
...
// and velB (usSource=2 -> usDestination 0x0207 EG1 DECAY; gm.dls authors no
// such connection, so this term is inert for this collection):
...
(`TableLookup_0x18e1c` reads the 256-entry WORD table at `0x1a9d8`, which is
**all-zero** in this build `[A]`, so the velocity term above is `0` whenever
no `usSource=2` connection scales that dimension. The structurally identical
table at `0x1a7d8` feeding ``voice+0x8``'s LFO path is likewise all-zero. Both
are inert in this binary.)
...
* The object pointed to by voice `+0x104` is the articulation block, not a
  separate structure: `+0x40` is LFO_FREQUENCY and `+0x20` onward is the EG1
  attack/decay/release/sustain block (xref:s2-4-3[§2.4.3],
  xref:s5-1-1[§5.1.1], xref:s5-1-2[§5.1.2]). **`[O]`**: `+0x0`, which feeds
  `voice+0x38` (EG2), is not laid out field-by-field.
...
* **`[O]`** The numeric time unit of ``0x19834``'s divisor constant `70`
  (xref:s5-6[§5.6]). The instructions do not prove it is milliseconds; the
  measured 70.0 ms cut time corroborates it without settling the conversion.
...
achievable gain, independent of the still-`[O]` absolute Q-format of
`raw_gain`:
...
Twenty-four distinct `[O]` items. Seven are **behaviourally significant** —
...
| xref:o4[4] | How `0x19c12`–``0x19c2a``'s two-table read reconciles with the measured pan curve | Cosmetic
...
| xref:o7[7] | The scaling arithmetic converting a raw 14-bit pitch-bend value and its RPN0 range into cents | Behaviourally significant
...
| xref:o10[10] | The disassembly address of the RPN1/RPN2 (fine/coarse tune) consumer | Cosmetic
| xref:o11[11] | The consumer of the per-part 16×12 tuning grid at `device+0xfbc` | Behaviourally significant
...
| xref:o16[16] | The absolute fixed-point unit of the raw gain field at `voice+0x2c`/`+0x30` | Cosmetic
...
[#o4]
==== How `0x19c12`–``0x19c2a``'s two-table read reconciles with the measured pan curve

*Known.* The instruction sequence is `[A]`: `gainA = *(0x1c1cc − pan*4)` (√-law table, reverse-indexed) and `gainB = table_1c9d0[pan]` (squared law). The measured curve (xref:s3-6[§3.6]) is 0 dB on both channels across CC10 48–80 and floors at ≈−20.2 dB, contradicting the formula's mid-pan dip and −96 dB extremes `[M: probe 25]`

*Not known.* Whether the disassembly reading is wrong, or correct for a different build than the one that produced the reference captures.

*How it would be settled.* A byte-level re-trace of `0x19bfe`–`0x19c2a` against the exact binary that produced the reference audio.

*Significance.* Cosmetic for output fidelity — the measured curve is what an implementation must reproduce. Significant for confidence in the disassembly.
...
[#o7]
==== The scaling arithmetic converting a raw 14-bit pitch-bend value and its RPN0 range into cents

*Known.* The defaults are confirmed at their constructor: centre 8192, range 200 cents `[A:0x16e3d]` `[A:0x16e44]`. The standard MIDI formula `(raw14 − 8192)/8192 × rangeCents` is consistent with every value observed.

*Not known.* The specific multiply/divide sequence performing the scaling.

*How it would be settled.* Trace forward from the pitch-bend controller's `+0x18` field to its first consumer inside `0x18ef4`; or render a bend sweep at several RPN0 range settings and check the pitch shift against the linear formula.

*Significance.* **Behaviourally significant** — a wrong formula gives an audibly wrong bend response curve.
...
[#o10]
==== The disassembly address of the RPN1/RPN2 (fine/coarse tune) consumer

*Known.* Write sites and values are confirmed (xref:s4-4[§4.4]). The **behaviour** is settled by measurement: RPN1/RPN2 are sampled once at note-on into the voice's fixed pitch base and do not retune a sounding voice, unlike Pitch Bend — a mid-note RPN2 change leaves the rendered fundamental flat `[M: probe 23]`

*Not known.* Only the VMA of the code that reads `device+0xf7c`/`+0x12bc`

*How it would be settled.* Locate the instruction inside the pitch chain that reads those fields at note-trigger time, by analogy with the confirmed pitch-bend chain (xref:s3-3-2[§3.3.2])

*Significance.* Cosmetic — the behaviourally significant question is measured and settled.

[#o11]
==== The consumer of the per-part 16×12 tuning grid at `device+0xfbc`

*Known.* Write site and shape are confirmed: 12 signed semitone-class offsets per part, `value = byte − 0x40`, written by the GS `DT1` fallthrough case `[A:0x13797]`

*Not known.* No confirmed reader anywhere in the control plane. The identification as Roland GS "Scale Tuning" is `[I]` from the grid's shape and centre normalization, not from a label or an independently consulted specification.

*How it would be settled.* Render a probe sending SysEx address `40 1x 40` with distinct per-semitone-class values and check for a semitone-class-selective pitch shift.

*Significance.* **Behaviourally significant if real content uses it** — niche in practice.

[#o12]
==== The layout of the object at voice `+0x104` beyond its four exercised offsets, and the scheduling semantics of the `+0x118`/`+0x11c` timestamp pair

*Known.* The object is the articulation block: `+0x40` is LFO_FREQUENCY, `+0x20` onward the EG1 block (xref:s5-1-2[§5.1.2]). The timestamp pairs are read and written by the envelope-advance machinery.

*Not known.* The layout of `+0x0` (which feeds EG2) field-by-field; the exact algorithm the `+0x118` pair implements.

*How it would be settled.* A full instruction trace of `0x19644` and its callees.
...
[#o16]
==== The absolute fixed-point unit of the raw gain field at `voice+0x2c`/`+0x30`

*Known.* The internal `<<8>>5` (= `<<3`) scaling between the field's rest and in-loop representations is confirmed (xref:s6-4-5[§6.4.5])

*Not known.* Whether the physical unit is Q16.16 relative to full scale, or something else.

*How it would be settled.* Not resolvable from the render path alone. The full gain chain — hundredths-of-a-dB sums, `10^(atten/2000)` — is already specified in xref:s3-5[§3.5] and supplies the numeric value an implementer needs.

*Significance.* Cosmetic — redundant with the specified gain chain.
