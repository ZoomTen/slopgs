...
with the driver allocating 1024-frame buffers `[A:0x184c0]`. Wired up at that
cadence the model breaks the probe xref:SPEC.adoc#s5-5[S5.5] bounds it with.
Corpus mean / `20_voice_count`: 256 frames -31.0087 / -17.25, 1024 frames
-31.1163 / -22.95, the fitted 2048 -31.1440 / -24.14, pre-segment build
-31.1483 / -24.13. The recovered cadence is therefore not usable as one, which
means something else in Branch B is wrong. Likeliest suspect: `need`. A voice
...
another batch. If the driver increments its reserveCount at MARK time rather
than at recycle time, one batch per saturation event falls out at any cadence,
which is what S5.5's measurement shows. Untested -- next step on this item.
