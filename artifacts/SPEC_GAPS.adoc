...
**The segment LENGTH ships at the measured value against the corpus gate, and
that is a deliberate, recorded gate failure.** Direct measurement puts the
segment at ~512 frames (23.2 ms) -- see the probe numbers above. The corpus
sweep puts it at 128 (5.8 ms): -31.1897/0.9249 at 128, -31.1552/0.9249 at 256,
-31.0757 at 384, -30.9116 at 512, -30.1227 at 768, -29.3281 at 1024. By
xref:../CLAUDE.adoc[CLAUDE.adoc]'s rule 512 should not ship; it costs 0.24 dB of
corpus mean. It ships because three things say the corpus is the instrument that
is wrong here rather than the value:
...
* 512's mean is dragged down by exactly TWO sparse single-note probes,
  ...
  **Why those two move that far is not diagnosed.** That is the single piece of
  evidence that would settle 128 vs 512 and it is still owed -- do it before
  re-fitting anything.
* 512 wins every item that exercises the gain path this models: `25_pan_law`
  ...
  `07_pan_volume` -1.28.
* On the dropout metric the ranking inverts. Field-only `dead` totals: 2260 ms
  for the one-pole build, 3975 for 512, 5450 for 128. The corpus optimum is the
  WORST of the three at inventing digital silence the reference does not have,
  and the spectral mean does not see that at all. Both new builds are worse than
  the one-pole on this, which is its own unexplained item.
...
misplaced by up to one segment in either direction. A shorter segment scores
better by under-applying the mechanism, not by being right. The gating tests
improve at every length tested, so what is open is how far to apply this, never
whether.
