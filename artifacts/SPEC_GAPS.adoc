= SPEC_GAPS -- Discrepancy and Gap Ledger
:sectnums:
:sectnumlevels: 5
:toc:
:toclevels: 4
...
* **#10 (tremolo)**: SPEC.md xref:SPEC.adoc#part7[Part 7] reports the `(usSource=1 LFO, usControl=0x0081
...
* **Row 7 (voice-stealing top-up) is RESOLVED** -- see #7 below. The
  ...
  against SPEC.md's own xref:SPEC.adoc#s5-5[S5.5] figure. See `FITTED.md` xref:FITTED.adoc#entry11[Entry 11].
* **Row 19 is PARTIALLY resolved, and was never as clean a case as this
  ...
  (see the correction blocks under #19 below, and `FITTED.md` xref:FITTED.adoc#entry9[Entry 9]) --
  ...
  same-session attempt to fix that (a linear ramp, `FITTED.md` xref:FITTED.adoc#entry13[Entry 13]) was
  ...
  that rule differently (a geometric ramp, `FITTED.md` xref:FITTED.adoc#entry12[Entry 12]) was measured
...
[#item1]
== The public ABI has no way to inject a SysEx message

**Path:** `SPEC.md` xref:SPEC.adoc#part1[Part 1] xref:SPEC.adoc#s1-5-3[S1.5.3] (the fixed ABI) vs. xref:SPEC.adoc#part4[Part 4] (SysEx: GM
...
through the public ABI at all, despite SPEC.md xref:SPEC.adoc#part4[Part 4] specifying SysEx as a
...
[#item2]
== Section 2.9.2 ("Wave-object fields") contradicts Section 3.2's own model

**Path:** `SPEC.md` xref:SPEC.adoc#s2-9-2[S2.9.2] vs. xref:SPEC.adoc#s3-2[S3.2]/xref:SPEC.adoc#s2-5[S2.5].
...
[#item3]
== Section 2.5 calls the velocity-depth sentinel "disabled"; the S3.5 formula says it means "full effect"

**Path:** `SPEC.md` xref:SPEC.adoc#s2-5[S2.5] ("default velocity->attenuation curve is disabled")
...
this class of synth. Verified this is moot for `gm.dls` specifically: xref:SPEC.adoc#s7-a-5[S7.A.5]
...
[#item4]
== S3.10's consolidated pseudocode mixes hundredths-of-dB and tenths-of-dB units with no shown conversion -- now SETTLED empirically (`[M: probe 07]`), against the literal (no x10) reading, previously only plausibility-argued

**Path:** `SPEC.md` xref:SPEC.adoc#s3-10[S3.10] (consolidated note-on pseudocode) vs. xref:SPEC.adoc#s1-4-4[S1.4.4] (units)
...
[#item5]
== Section 3.3.5's own pseudocode and its own prose disagree about which
   offset convention `region[N]` uses

**Path:** `SPEC.md` xref:SPEC.adoc#s3-3-5[S3.3.5].
...
[#item6]
== `smf.c`'s design is unconstrained by SPEC.md (by its own S1.5.4 statement) -- documented design choices
...
* `msgs_load_smf` calls a full synth/voice reset before loading, so a newly
  ...
  specified either way by SPEC.md (xref:SPEC.adoc#s1-5-4[S1.5.4] disclaims any original-driver
...
[#item7]
== Voice pool: single flat 54-voice pool instead of the 48-primary + 6-reserve two-tier design
...
splits SPEC.md xref:SPEC.adoc#s5-7[S5.7]'s two steal comparators back apart:
...
**What is fitted, not recovered:** the top-up CADENCE. SPEC.md xref:SPEC.adoc#s5-4[S5.4] states
...
`FITTED.md` xref:FITTED.adoc#entry11[Entry 11].

**Remaining discrepancy, not papered over:** against SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s own
...
SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s own `[M]` figure bounds the period from the other side and
...
(23-46 ms); 2048 sits above it. Full fit record in `FITTED.md` xref:FITTED.adoc#entry14[Entry 14].
...
surviving / 32 cut**, matching SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s `[M]` figure. The off-by-one
was the too-fast cadence marking one extra voice, not a pool-size error.

The original entry follows as history.
____

**Path:** `SPEC.md` xref:SPEC.adoc#part5[Part 5] xref:SPEC.adoc#s5-2[S5.2]-xref:SPEC.adoc#s5-5[S5.5].
...
[#item8]
== Envelope model: per-sample linear-attack / exponential-decay-and-release, not a reproduction of the original's own (unrecovered) update mechanism

**Path:** `SPEC.md` xref:SPEC.adoc#s3-4-2[S3.4.2], xref:SPEC.adoc#s6-6[S6.6] (both explicitly mark the original's own
...
[#item9]
== Pan law L/R channel assignment -- now SETTLED by measurement (`[M: probe 07]`)

**Path:** `SPEC.md` xref:SPEC.adoc#s3-6[S3.6], explicitly marked `[O]` for which physical output
...
normalised sweep table in `FITTED.md` xref:FITTED.adoc#entry7[Entry 7]: L is at its own maximum,
...
`src/engine/voice.c`'s pan-law comment and `FITTED.md` xref:FITTED.adoc#entry7[Entry 7] for the
current gain formula (`g_table_lin`, both channels, re-centered).

**Also settled by the same probe:** the *other* half of SPEC.md xref:SPEC.adoc#s3-6[S3.6]'s
...
`FITTED.md` xref:FITTED.adoc#entry7[Entry 7].
...
[#item10]
== LFO->pitch (vibrato) is now applied (`[M: probe 06]`, FITTED.md Entry 6); LFO->attenuation, real-time pitch-bend-to-EG2 routing, key-follow, and the GS per-part tuning grid remain parsed but not applied
...
`FITTED.md` xref:FITTED.adoc#entry6[Entry 6], tagged `[M: probe 06]`). Per-instrument rate (from
...
`FITTED.md` xref:FITTED.adoc#entry6[Entry 6] section 3 for the full measurement and root cause (a
...
vibrato without reintroducing the confirmed regression (`FITTED.md` xref:FITTED.adoc#entry6[Entry 6]
section 4).

**`usSource=3` (key-follow) is now applied too — RESOLVED 2026-07-26.** Both
decay rows of SPEC.md xref:SPEC.adoc#s2-4-3[S2.4.3]'s own "Source = 3" table (`0x0207` EG1 decay,
`0x030b` EG2 decay) are stored and consumed at note-on
(`voice.c`'s `decay_tc_keyfollow`); see SPEC.md xref:SPEC.adoc#s2-4-3-2[S2.4.3.2] for the measurement.
...
[#item11]
== RCV CHANNEL "Part" indirection is not modeled

**Path:** `SPEC.md` xref:SPEC.adoc#s4-2-2[S4.2.2]/xref:SPEC.adoc#s4-6-1[S4.6.1]/xref:SPEC.adoc#s4-8[S4.8]/xref:SPEC.adoc#t8[T.8] (the `USE RHYTHM PART`/`RCV
...
default `RCV CHANNEL` table `SPEC.md` xref:SPEC.adoc#t8[T.8] documents byte-for-byte
...
[#item12]
== Wave-level 8-bit PCM storage path deviates from "reference in place"

**Path:** `SPEC.md` xref:SPEC.adoc#s1-5-5[S1.5.5] ("sample data is referenced, not copied").
...
xref:SPEC.adoc#s6-2-2[S6.2.2]) are not implemented at all, matching SPEC.md's own note that these
are real but structurally unreachable for this specific sample collection.

[#item13]
== Numeric tables built but not wired into the signal path
...
(since `FITTED.md` xref:FITTED.adoc#entry6[Entry 6], xref:item10[item 10] above) the 256-entry sine LFO table
...
driver are themselves marked `[O]`/unrecovered by SPEC.md itself (xref:SPEC.adoc#s2-7-2[S2.7.2]'s
`flagByte` origin, xref:SPEC.adoc#s6-6[S6.6]'s ramp/envelope cadence).

[#item14]
== Event scheduling: no timestamp-keyed pending-controller queues

**Path:** `SPEC.md` xref:SPEC.adoc#s4-2-1[S4.2.1]/xref:SPEC.adoc#s4-7[S4.7] (six per-controller scheduled queues with
...
[#item15]
== `exp_coef`'s per-sample coefficient: retuned from a 1/e time constant to a "100dB-over-`seconds`" calibration -- SPEC.md leaves the exact number `[O]`
...
data with no fudge factor. Shipped; see SPEC.md xref:SPEC.adoc#s2-4-3-2[S2.4.3.2] and xref:item10[item 10] above.
...
still matches. See FITTED.md xref:FITTED.adoc#entry15[Entry 15] section 4b.
____

**Path:** `SPEC.md` xref:SPEC.adoc#s3-4-1[S3.4.1] (timecent->duration formula, confirmed `[A]`) vs.
...
(confirmed directly against the file's raw bytes, matching SPEC.md xref:SPEC.adoc#s2-4-3[S2.4.3]'s
...
real consumption code (SPEC.md xref:SPEC.adoc#part5[Part 5] `+0x13c`/`0x194da`/`0x19644`) lives
...
0.0005`, an arbitrary -66dB) with SPEC.md xref:SPEC.adoc#part5[Part 5]'s own confirmed
...
values at full velocity). SPEC.md's own xref:SPEC.adoc#s3-5[S3.5]/xref:SPEC.adoc#s3-10[S3.10] pseudocode sums only the
...
[#item16]
== Where latched RPN1/RPN2 tuning enters the pitch chain -- SETTLED by measurement (`[M: probe 30]`), the spec was silent
...
**Path:** `SPEC.md` xref:SPEC.adoc#s3-3-2[S3.3.2] (the note-on cents sum) vs. xref:SPEC.adoc#s3-3-3[S3.3.3] (the clamp) vs.
...
[#item17]
== Whether a live pitch bend re-enters the clamped sum -- SETTLED by measurement (`[M: probe 30]`), SHIPPED
...
**Path:** `SPEC.md` xref:SPEC.adoc#s3-3-2[S3.3.2]/xref:SPEC.adoc#s3-3-4[S3.3.4] (note-on only) vs. xref:SPEC.adoc#s4-4[S4.4] (bend is
...
[#item18]
== Expression (CC11) behaviour under fast gating is unmeasured, and the reference does something our smoothing model cannot produce

**Path:** `FITTED.md` xref:FITTED.adoc#entry4[Entry 4] (`GAIN_SMOOTH_ALPHA`, `[F:fitted]`) vs.
`SPEC.md` xref:SPEC.adoc#s6-6[S6.6] (ramp/envelope update cadence, `[O]`).

xref:FITTED.adoc#entry4[Entry 4]'s 12 ms gain-smoothing time constant was fit against slow CC7/CC11
...
[#item19]
== Control changes are RAMPED -- and SPEC.md S6.6 documents the mechanism as `[A]`; we simply never implemented it
...
`FITTED.md` xref:FITTED.adoc#entry9[Entry 9] for the fit, the full corpus A/B (isolated via
...
one-pole exponential (xref:FITTED.adoc#entry4[Entry 4]), per this task's own explicit instruction
...
xref:FITTED.adoc#entry8[Entry 8]'s surrounding notes) said the ramp machinery was `[O]`/unrecovered in
SPEC.md. **That is wrong and was my error, not the spec's.** SPEC.md xref:SPEC.adoc#s6-6[S6.6]
...
the phase-step RAMP mechanism is `[A]` (confirmed, xref:FITTED.adoc#entry9[Entry 9] above ships it),
...
instead of xref:FITTED.adoc#entry9[Entry 9]'s fixed rate-with-ceiling design). It clearly moved
...
`FITTED.md` xref:FITTED.adoc#entry12[Entry 12].
...
they were never touched by this attempt, remain the shipped fit (xref:FITTED.adoc#entry9[Entry 9]),
...
rule keyed to bend magnitude (as xref:FITTED.adoc#entry9[Entry 9]'s own rate-with-ceiling design
already is) rather than a fixed ratio -- see `FITTED.md` xref:FITTED.adoc#entry12[Entry 12] section 5.
____

[#item19-original-entry]
=== Original entry

**Path:** `SPEC.md` xref:SPEC.adoc#s6-6[S6.6] (ramp/envelope update cadence, `[O]`) vs.
`FITTED.md` xref:FITTED.adoc#entry4[Entry 4] (`GAIN_SMOOTH_ALPHA`, a fitted 12 ms one-pole).
...
Endpoints agree, so the bend *scaling* is right (that is xref:FITTED.adoc#entry8[Entry 8]'s result).
...
`SPEC.md` xref:SPEC.adoc#s6-6[S6.6] marks precisely this machinery `[O]`, and xref:SPEC.adoc#part5[Part 5]'s `+0x13c`
...
[#item20]
== EG2 (the pitch envelope) -- RESOLVED 2026-07-26, implemented and shipped
...
**Release is NOT rate-clamped.** SPEC.md xref:SPEC.adoc#part7[Part 7] records that the choke/steal
...
recomputed at exactly that cadence. SPEC.md xref:SPEC.adoc#s6-6[S6.6] marks the envelope
...
[#item20-original]
== EG2 (the pitch envelope) is parsed and then discarded -- `[A]` in SPEC.md, never applied

**Path:** `SPEC.md` xref:SPEC.adoc#s2-4-3[§2.4.3] "Source = 5 (EG2)" vs. `src/engine/voice.c`
`voice_update_pitch`.
...
* Application point: the `+ 0` slot in `voice_update_pitch`, which already
  ...
  will land correctly.
* **Caution:** SPEC.md xref:SPEC.adoc#s6-6[S6.6] warns the envelope generator's own update cadence
...
[#item21]
== Ordinary-note-off release is too short against isolate references -- fitted floor SHIPPED, superseded 2026-07-28: the real defect is in the GAIN path, see the resolution addendum below

**Path:** `SPEC.md` xref:SPEC.adoc#s5-6[S5.6]/xref:SPEC.adoc#s3-8-2[S3.8.2] (the only documented minimum-release
...
low-register resolution floor), is in `FITTED.md` xref:FITTED.adoc#entry10[Entry 10].
...
SPEC.md xref:SPEC.adoc#part5[Part 5] `+0x13c`, `0x194da`/`0x19644`, and `compute_release_target` /
...
[#item21-addendum]
== Item 21 addendum 2026-07-27: the shape is SETTLED by `[M: probes 44/45]` -- it is a LINEAR ramp to zero, not a stretched exponential -- but implementing it alone REGRESSES the corpus, so the fitted floor stays
...
== Item 21 addendum 2026-07-28: disassembly of the release path -- the ramp is a fixed RATE `[A:0x197dc]`, note-off is scheduled sample-accurately `[A:0x16cac]`, and the earliness is NOT the event drain

Read against `artifacts/swmidi.sys` (PE32, image base `0x10000`, so SPEC.md's
...
arg/0x46)` -- SPEC.md xref:SPEC.adoc#s5-6[S5.6]'s 70 ms figure, confirmed as a
...
**Also closed along the way:** SPEC.md's open question #2 (the frame count
...
[#item21-resolved]
== Item 21 RESOLVED 2026-07-28 by `[A:0x18fba]`: there is no release floor. The "linear ramp" is the gain interpolator's segment quantisation, and it is why the release starts early
...
the segment end. That also subsumes SPEC_GAPS #19's "render.c's gain smoother
...
per-call frame count from the KS descriptor (~512 frames), which -- per SPEC.md
...
[#item21-reap]
== Item 21 addendum: `+0x13c` is the bare EG, not a gain-inclusive level -- `[A:0x194da]` corrects SPEC.adoc S5.7's `[I]`
...
[#item22]
== CC121 (Reset All Controllers) does not reset Channel Volume -- RESOLVED 2026-07-26 by `[M: probe 37]`, SHIPPED

**Path:** `SPEC.md` xref:SPEC.adoc#s4-3[S4.3]'s CC121 row, `[A:0x1351f]`: "re-schedules Volume=100,
...
[#item23]
== A real, multi-dB EG1 decay on HIGH-sustain instruments that this project's decay model cannot produce, and no authored parameter predicts which instruments show it -- OPEN
