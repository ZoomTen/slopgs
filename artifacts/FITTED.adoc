= FITTED -- fitted-value ledger
:sectnums:
:sectnumlevels: 5
:toc:
:toclevels: 4

[#fitted-md-fitted-value-ledger-what-f-fitted-means]
== What `[F:fitted]` means
...
[#fitted-md-fitted-value-ledger-2026-07-25-harness-re-baseline-db-figures]
...
[#entry1]
== EG1 decay-segment rate multiplier (Acoustic Grand Piano probe) -- FIT ATTEMPTED, NOT SHIPPED, SUPERSEDED BY ENTRY 15
...
everywhere else. Now shipped properly as xref:entry15[Entry 15] / SPEC.md xref:SPEC.adoc#s2-4-3-2[S2.4.3.2].
____
...
* **Probe/reference:** `probes/04_envelope.mid` vs. `probe-results/04.flac`,
  ...
  recorded in `SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item15[item 15]).
...
`SPEC.md` xref:SPEC.adoc#part5[Part 5] `+0x13c`, functions **`0x194da`** / **`0x19644`** -- the
...
`SPEC_GAPS.md` xref:SPEC_GAPS.adoc#item15[item 15] already resolved well via xref:SPEC.adoc#s5-6[S5.6]'s independent 70ms
...
[#entry2]
== pitch-LFO (vibrato) rate + depth-application scale (CC1/mod-wheel probe) -- FIT ATTEMPTED, NOT SHIPPED
...
`SPEC.md` xref:SPEC.adoc#part2[Part 2] xref:SPEC.adoc#s2-4-2[§2.4.2]/xref:SPEC.adoc#s2-4-3[§2.4.3] -- the **depth VALUES** this reads
...
[#entry3]
== pitch-LFO retry -- per-instrument rate/delay via `lfo_freq_tc`/`lfo_delay_tc`, both depth connections -- FIT ATTEMPTED, STILL NOT SHIPPED
...
* **`src/engine/dls.c`/`src/engine/dls.h`**: `art1` `usSource==1(LFO)`, `usDestination==0x0003
  ...
  `[-1200,1200]`" formula SPEC.md xref:SPEC.adoc#s2-4-2[S2.4.2]/xref:SPEC.adoc#s2-4-3[S2.4.3] already confirms `[A]` for
...
* **LFO start delay, per-instrument**: `lfo_delay_tc` (already parsed,
  ...
  helper directly (SPEC.md xref:SPEC.adoc#s2-4-3[S2.4.3]: destination `0x0105` is routed through
...
[#entry4]
== gain-smoothing time constant for CC7/CC11 Expression gliding (`GAIN_SMOOTH_ALPHA` in src/engine/render.c and src/engine/voice.c) -- SHIPPED
...
still the best available stand-in for SPEC.md xref:SPEC.adoc#s6-6[S6.6]'s `[O]` real ramp), but a
...
[#entry5]
== pan-law interpolation shape between probe-25 anchors -- SUPERSEDED
...
`FITTED.md` xref:entry7[Entry 7] for the replacement, the saturation evidence, and the
...
[#entry6]
== pitch-LFO (vibrato) -- SHIPPED, `[M: probe 06]`, gated on direct rate/depth measurement not on probe 06's own spectral score
...
[#entry7]
== pan law replaced with centre-normalised `g_table_lin` sqrt-table (probe 07) -- SHIPPED
...
`g_table_lin` (SPEC.md xref:SPEC.adoc#s3-6[S3.6]'s `gainA`), reverse- and direct-indexed, instead
...
----
  ...
  L loud at CC10=0 (hard left) confirms Left = `gainA` (SPEC_GAPS.md xref:SPEC_GAPS.adoc#item9[S9], now
  ...
  of SPEC.md xref:SPEC.adoc#s3-6[S3.6]'s disassembly reading) is REFUTED by the same probe: it
...
presumed to live in the unrecovered gain-register scaling, SPEC.md xref:SPEC.adoc#s6-4-5[S6.4.5]
...
[#entry8]
== pitch bend / LFO applied outside the CentsToRatio clamp -- SHIPPED, `[M: probe 30]`, not a fit
...
[#entry9]
== phase-step ramp (`PITCH_RAMP_RATE_FRAC_PER_MS` / `PITCH_RAMP_MAX_MS` in src/engine/voice.c) -- SHIPPED, SPEC_GAPS.md #19

**Status: `[F:fitted, SHIPPED]`.** Implements SPEC.md xref:SPEC.adoc#s6-6[S6.6]/xref:SPEC.adoc#s6-4-1[S6.4.1]'s `[A]`
...
[#entry9-step4]
=== HueArme isolate (SPEC_GAPS.md #19's own acceptance test), bend-rate-conditioned
...
Same as xref:entry4[Entry 4]'s gain smoother: SPEC.md xref:SPEC.adoc#s6-6[S6.6] says the caller-side slope
...
[#entry10]
== release-duration floor (`RELEASE_FLOOR_S` in src/engine/voice.c) -- SHIPPED
...
* **What this stands in for:** SPEC.md xref:SPEC.adoc#s5-6[S5.6] and xref:SPEC.adoc#s3-8-2[S3.8.2] both state that the
...
SPEC.md xref:SPEC.adoc#part5[Part 5] `+0x13c`, `0x194da`/`0x19644` (the real EG consumption code,
...
[#entry11]
== voice-stealing reserve top-up cadence and reserve size (`TOPUP_RESERVE_COUNT`, `TOPUP_PER_SUBCHUNK` in src/engine/voice.c and src/engine/render.c) -- CADENCE HALF SUPERSEDED BY ENTRY 14
...
This entry covers the fitted PARAMETERS of a SPEC.md xref:SPEC.adoc#s5-2[S5.2]-xref:SPEC.adoc#s5-5[S5.5]-documented
...
the next event. `TOPUP_RESERVE_COUNT=6` reproduces SPEC.md xref:SPEC.adoc#s5-2[S5.2]-xref:SPEC.adoc#s5-5[S5.5]'s own
...
* **Cadence -- the load-bearing fit, two cadences tried and measured:**
  SPEC.md xref:SPEC.adoc#s5-4[S5.4] states `TopUpReserve` (`0x12b6a`) runs "exactly once per call
...
* **`TOPUP_PER_SUBCHUNK=1` is kept as a compile-time A/B switch**, retaining
  ...
  on probe 21.
* **Against SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s own `[M]` figure** (80 simultaneous note-ons
...
47-vs-48 / 33-vs-32 one-voice discrepancy against SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s own
measured figure.
...
[#entry12]
== geometric pitch-bend ramp (`PITCH_RAMP_K_PERIODS`) -- FIT ATTEMPTED, NOT SHIPPED
...
Same as xref:entry9[Entry 9]: SPEC.md xref:SPEC.adoc#s6-6[S6.6]/xref:SPEC.adoc#s6-4-1[S6.4.1]'s `[A]` mechanism (linear ramp,
...
[#entry13]
== linear gain ramp replacing the one-pole (`GAIN_RAMP_MAX_MS`) -- FIT ATTEMPTED, NOT SHIPPED
...
SPEC.md xref:SPEC.adoc#s6-6[S6.6]/xref:SPEC.adoc#s6-4-1[S6.4.1] actually documents `[A]` (a linear step re-derived from
...
Same as xref:entry9[Entry 9]/xref:entry4[Entry 4]: SPEC.md xref:SPEC.adoc#s6-6[S6.6]/xref:SPEC.adoc#s6-4-1[S6.4.1]'s linear-ramp mechanism is
...
[#entry14]
== reserve top-up tick period (`TOPUP_INTERVAL_FRAMES` in src/engine/voice.c) -- SHIPPED

**Status: mechanism `[A]` (SPEC.md xref:SPEC.adoc#s5-4[S5.4]), period `[F:fitted, SHIPPED]`.**
...
Root cause was the cadence, not the mechanism. SPEC.md xref:SPEC.adoc#s5-4[S5.4] says the real
...
[#entry14-step2]
=== Lower bound, from SPEC.md's own measurement

SPEC.md xref:SPEC.adoc#s5-5[S5.5] `[M]`: 80 note-ons with no note-off leave 48 sounding, 32 cut =
...
fast-release time** (SPEC.md xref:SPEC.adoc#s5-6[S5.6]'s measured 70.0 ms). 2048 frames
...
cut**, matching SPEC.md xref:SPEC.adoc#s5-5[S5.5]'s `[M]` figure. This closes the "47 surviving /
...
SPEC.md xref:SPEC.adoc#s5-4[S5.4]/xref:SPEC.adoc#s5-5[S5.5]'s mechanism is `[A]`; the *period* is the `0x13054` service
routine's own timer, which SPEC.md marks `[O]` (xref:SPEC.adoc#s6-7[S6.7]: "block-cadenced, exact
...
[#entry15]
== EG1/EG2 decay-time key-follow (`decay_tc_keyfollow` in src/engine/voice.c) -- SHIPPED, `[M: field/town.mid]` + `[I]` normalization
...
[#entry16]
== release finish detection on the AUDIBLE level (`AUDIBLE_FLOOR` in src/engine/voice.c) -- SHIPPED, `[I]` reading of `+0x13c`

**Status: `[I]`, not a curve fit.** SPEC.md xref:SPEC.adoc#s5-7[S5.7] reads `+0x13c` -- the field
...
`48/48 voices survive saturation` (SPEC.md xref:SPEC.adoc#s5-5[S5.5] `[M]`) at every floor value.
